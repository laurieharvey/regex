...
void cmdline::add_optional(const std::string& arg,
                           ...
                           const std::vector<std::any>& options) {
  args_[var] = def;
        ...
        if (i + 1 >= argc) {
          ...
          if (val.starts_with("-")) {
            ...
            if (opt_iter == std::cend(optionals_)) {
              throw exception(std::string("Unknown optional ") + arg + std::string(" provided") +
...
static std::ostream& operator<<(
    std::ostream& os,
    const std::vector<std::tuple<std::string, cmdline::type, std::string>>& positionals) {
