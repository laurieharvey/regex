...
std::shared_ptr<regex::fa> compile(std::basic_stringstream<language::character_type> expression,
                                   compile_flag flag) {
  if (flag == compile_flag::nfa) {
    return compile_nfa(std::move(expression));
  } else {
...
std::shared_ptr<regex::nfa> compile_nfa(
...
std::shared_ptr<regex::dfa> compile_dfa(
    std::basic_stringstream<language::character_type> expression) {
