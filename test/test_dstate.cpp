...
TEST( dstate, copy )
{
    ...
    regex::dstate::copy( state_21, state_11 );
    ...
    EXPECT_EQ( state_21->execute( "a" ),  regex::match::accepted );
    EXPECT_EQ( state_21->execute( "b" ),  regex::match::rejected );
    EXPECT_EQ( state_21->execute( "c" ),  regex::match::accepted );
    EXPECT_EQ( state_21->execute( "ad" ), regex::match::rejected );
    EXPECT_EQ( state_21->execute( "ae" ), regex::match::accepted );

    auto loop = std::make_shared<regex::dstate>( regex::state::context::accepting );

    regex::dstate::connect( loop, loop, 'a' );
    regex::dstate::copy( loop, loop );
