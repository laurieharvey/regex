...
class row {
 ...
 public:
  ...
  reference operator[](size_type col) { return m_elements[col]; }

  iterator begin() { return std::begin(m_elements); }

  iterator end() { return std::end(m_elements); }

  const_iterator begin() const { return std::cbegin(m_elements); }

  const_iterator end() const { return std::cend(m_elements); }
};
...
class table {
 ...
 public:
  ...
  iterator find(const key_type& key) { return r_.find(key); }

  auto size() const { return std::pair<int, int>{s_.size(), r_.size()}; }

  schema get_schema() const { return s_; }

  reference operator[](size_type pos);

  iterator begin() { return std::begin(r_); }

  iterator end() { return std::end(r_); }

  const_iterator begin() const { return std::cbegin(r_); }

  const_iterator end() const { return std::cend(r_); }
};
}  // namespace regex